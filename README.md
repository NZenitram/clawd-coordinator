...
**Status:** V2 complete (7 phases). Production-ready with SQLite persistence, per-agent concurrency, workspace isolation, REST API, and MCP server integration.
...
- `agentTokens` — Per-agent tokens (optional, for per-agent auth in V2+)
...
- [x] **Phase 1:** FATAL operational fixes (task error on disconnect, validation)
- [x] **Phase 2:** Protocol hardening, message deduplication, per-agent tokens, budget tracking
- [x] **Phase 3:** Agent health checks, stale agent eviction
- [x] **Phase 4:** Per-agent concurrency control (`--max-concurrent`)
...
- `sql.js` for SQLite (in-memory), with better-sqlite3 option for production
